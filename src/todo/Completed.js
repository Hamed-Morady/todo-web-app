...
export default function Completed() {
  ...
  return (
    <div className="data-task">
      ...
      <div className="show-tasks" ref={menuRef}>
        {completedTasks.length > 0 ? (
          completedTasks.map((ts, index) => (
            <div key={index} className="ts">
              <div className="icon-menu">
                <div className="data-box-task ts-section-date">{ts.data}</div>
              </div>
              <div className="ts-body">
                ...
                <div className="task-box-btn">
                  <button className="done" onClick={()=>unDoneHandler(ts.id)}>Undo</button>
                  <button className="done done-none" onClick={()=>deleteBtnHandler(ts.id)}>Delete</button>
                </div>
              </div>
            </div>
          ))
